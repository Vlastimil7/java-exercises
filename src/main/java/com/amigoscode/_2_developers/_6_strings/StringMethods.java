...
public class StringMethods {
    ...
    public static String searchString(String text, String keyword) {
        ...
        boolean search = text.contains(keyword);
        int position = text.indexOf(keyword);
        return String.format("contains: %s, indexOf: %d", search, position);
    }
    ...
    public static String splitDemo(String text, String delimiter) {
        ...
        for (int i = 0; i < split.length ; i++) {
            if (i < split.length -1 ) {
                resultBuilder.append("[").append(i).append("]").append(" ").append(split[i]).append("\n");
            } else {
                resultBuilder.append("[").append(i).append("]").append(" ").append(split[i]);
            }
    ...
    public static String checkStartEnd(String filename, String prefix, String extension) {
        ...
        boolean filePrefix = filename.startsWith(prefix);
        boolean fileExtension = filename.endsWith(extension);

        return "startsWith " + "'"+prefix+"': " + filePrefix + ", " + "endsWith " + "'"+extension+"': " + fileExtension ;
    }
