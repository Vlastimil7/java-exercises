...
class EmployeeDemo {
    public static void main(String[] args) {
        ...
        List<Employee> listOfEmployees = List.of(
                new FullTimeEmployee("Kali", 100_000),
                new FullTimeEmployee("Anna", 75_000),
                new ContractEmployee("Vendy", 520, 50),
                new ContractEmployee("Petr", 450, 120)
        );
        listOfEmployees.forEach(employees -> System.out.println(employees.getDetails()));
        System.out.println("Highest salary: " + getHighestPaid(listOfEmployees).getDetails());
    ...
    static Employee getHighestPaid(List<Employee> employees) {
        ...
        for (Employee employee : employees) {
            if (employee.calculatePay() > highestPaidEmployee.calculatePay()) {
